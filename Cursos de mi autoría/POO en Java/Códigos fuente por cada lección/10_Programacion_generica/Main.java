...
public class Main {
	public static void main(String[] args) {
		...
		Lista.add(palabra);
		palabra2 = (String)Lista.getFirst(); // para que esto funcione, Lista.getfirst debe estar casteado a String
		System.out.println("Lista.first = "+Lista.getFirst());
		
		
		
		
		// operador diamante, en este caso indicamos que la lista será de String's
		LinkedList <String>Lista2 = new LinkedList<>();
		Lista2.add(palabra);
		palabra2 = Lista2.getFirst();// nota que ya no hay warnings
		System.out.println("Lista.first = "+Lista2.getFirst());
		
		
		
		
		// en este caso indicamos que la lista será de 'Clase_Normal'-es
		LinkedList <Clase_Normal> Tasers = new LinkedList<>();
		Clase_Normal Taser1 = new Clase_Normal("Aturdidor",12.5,3.5,false);
		Clase_Normal Taser2 = new Clase_Normal("-",0,0,false);
		Tasers.add(palabra);// [!] Esto está mal, 'palabra' es clase "String", yo quiero objetos clase "Clase_Normal"
		Tasers.add(Taser1);//  [!] Esto está bien
		...
		// necesitaría yo hacer eso?. Porque LinkedList es una clase genérica, no sabe lo que vendrá
