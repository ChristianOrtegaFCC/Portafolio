...
xAux, yAux, zAux : Real;  //Variables auxiliares para la rotación.
var
  Form1: TForm1;
...
procedure TForm1.Button7Click(Sender: TObject);
    ...
    // (x, y, z):"valor" en 3D y (xP, yP):"referencia"  en 2D
    //
...
for i := 1 to 4 do
  begin
  ProyParalela( Cubo[i].x, Cubo[i].y, Cubo[i].z, P.x, P.y ); //Encuentra P.x y P.y
...
for i := 1 to 4 do
  begin
  ProyParalela( Cubo[i].x, Cubo[i].y, Cubo[i].z, P.x, P.y ); //Encuentra P.x y P.y
  ProyParalela( Cubo[i+5].x, Cubo[i+5].y, Cubo[i+5].z, Q.x, Q.y );
