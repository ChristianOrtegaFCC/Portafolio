...
void ordenando(int *A, int b, int e){
        if(b < e){
            int m = (b + e) / 2;
            ordenando(A, b, m);  // resolver (llamado recursivo para cortar una fracci�n del array)
            ordenando(A, m+1, e);// resolver (llamado recursivo para... )
            MERGE(A, b, m, e);// unir (ya que justo antes de esta, se encuentran llamados recursivos, el desempe�o de esta, tambi�n es de naturaleza recursiva)
        }
}
